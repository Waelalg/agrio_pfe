...
This appendix describes the role of the main PostgreSQL tables used by the Agrio platform. The focused ERD views are inserted in Chapter~\ref{chap:backend_database_iot}, and the complete class diagram is kept in Appendix~\ref{app:full_class_diagram}. The descriptions below are useful for defending the database design during the engineering discussion.
...
\section{Why this database supports a Digital Twin}
...
The strongest tables for defending the Digital Twin nature of the project are the Digital Twin tables, state-history tables, prediction tables, simulation tables, recommendation tables, control-command tables, irrigation-event tables, and execution-feedback tables. Together, these tables connect physical measurement to virtual representation and operational action.
