...
Deployment is a critical engineering part of the project. Agrio is composed of several services that must run together: database, backend, ML service, MAS service, frontend, scheduler, MQTT configuration, and mobile access support. Docker Compose is used to coordinate the documented prototype services.

\section{Docker Compose stack}

The Docker Compose stack includes PostgreSQL, the ML service, the MAS service, FastAPI backend, frontend, irrigation scheduler, ngrok for prototype mobile access, and Mosquitto MQTT. Each documented service has a clear responsibility and communicates through internal network links and environment variables. The exposed ports used during validation are consistent with Table~\ref{tab:docker_compose_ports}: frontend \texttt{3000}, backend \texttt{8000}, ML service \texttt{8001}, MAS service \texttt{8002}, PostgreSQL \texttt{5432}, Mosquitto MQTT \texttt{1883}, optional MQTT WebSocket \texttt{9001}, and ngrok inspection \texttt{4040}.
...
The communication flow can be divided into five groups. First, users access the web dashboard or mobile interface, which communicates with the FastAPI backend through REST requests. Second, the backend reads and writes persistent data in PostgreSQL. Third, the backend sends feature payloads to the ML service over HTTP and receives prediction responses. Fourth, the frontend or backend can send an explainable decision request to the MAS service through \texttt{/mas/decision}; the MAS can use ML prediction evidence and combines it with Digital Twin, weather, agronomy, water-balance, and safety evidence. Fifth, sensor telemetry, actuator commands, and WeMos status feedback are exchanged through MQTT topics.
...
\noindent\textbf{Update note:} this communication-flow figure should later be recaptured or redrawn to include the \texttt{mas-service} on port \texttt{8002}.
...
The MAS service runs on port \texttt{8002}. It exposes \texttt{/health} for service checks and \texttt{/mas/decision} for explainable irrigation decisions. In the documented Docker Compose stack, it depends on the ML service. Depending on the final integration path, it can be called by the frontend for direct decision inspection or by the backend as part of the recommendation workflow. The MAS service does not publish actuator commands; it returns a decision, explanation, confidence, risk level, recommended duration, evidence, and warnings.
...
MAS decision & Frontend/backend $\leftrightarrow$ MAS service & Send field context and receive explainable irrigation decision, evidence, and warnings. \\
...
The deployed prototype is divided into services with explicit responsibilities. PostgreSQL stores the \texttt{smart\_irrigation} schema and supports traceability from measurement to decision and feedback. The ML service loads Random Forest artifacts and exposes inference endpoints on port \texttt{8001}. The MAS service exposes explainable irrigation decision support on port \texttt{8002}. The backend exposes the FastAPI API, orchestrates MQTT ingestion, calls the ML service, can call or coordinate MAS decisions depending on the integration path, persists results, and publishes actuator commands. The frontend exposes the web dashboard on port \texttt{3000}. The scheduler runs as an internal worker and is not exposed as a public port. MQTT configuration connects backend ingestion and actuator workflows to the selected broker.

This organization keeps service boundaries clear. The frontend does not access PostgreSQL or MQTT directly. The ML service and MAS service do not publish actuator commands. Physical commands remain under the backend and scheduler workflow, which publishes the documented WeMos commands through MQTT and records feedback.
...
mas-service & Runs explainable multi-agent decision support on port 8002 & /health and /mas/decision response showing decision evidence \\
...
This chapter showed that Agrio is deployed as a reproducible multi-service prototype rather than as a single script or static dashboard. The deployment separates persistence, API orchestration, ML inference, MAS decision support, frontend access, scheduling, MQTT communication, and mobile testing support. This separation makes the validation workflow easier to reproduce while also making clear that production deployment would still require stronger security, monitoring, backups, and operational hardening.
